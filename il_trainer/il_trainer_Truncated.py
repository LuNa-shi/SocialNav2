...
sys.path.insert(0, "/root/swx/track2/Falcon")
try:
    import falcon  # 仅为反序列化ckpt可用
except Exception as e:
    print(f"[warn] falcon import failed: {e}")
...
SAMPLES_PER_EPOCH = 500000   # Total sample windows per epoch (controls __len__)
...
VAL_BATCH_SIZE = 64            # Smaller batch size for validation to avoid OOM
...
# Output paths
MODEL_SAVE_PATH = "il_model_0824.pth"
CHECKPOINT_DIR = "checkpoints0824"
...
# Stage-wise / grouped-LR configuration
FREEZE_BACKBONE = False            # Plan B: fully unfrozen backbone
USE_GROUPED_BACKBONE_LR = False     # Use a smaller LR for the backbone param group
LR_TOP = 1e-4                      # Global/top LR tuned down based on curves
WD_TOP = 1e-5                      # Weight decay for top modules
LR_BACKBONE = 1e-5                 # Smaller LR for backbone
WD_BACKBONE = 1e-6                 # Slightly smaller WD for backbone

# Learning rate scheduler configuration
# Options: 'plateau' (ReduceLROnPlateau on val_loss) or 'cosine' (Warmup + CosineAnnealing)
...
WARMUP_EPOCHS = 3           # 1–3 recommended (use 3 for batch_size=256)
WARMUP_START_FACTOR = 0.2   # start at 20% of base lr
COSINE_MIN_LR = 1e-6        # anneal down to this
...
class ImitationLearningPolicy(nn.Module):
    ...
    def _load_mixed_pretrained_weights(self):
        ...
        habitat_path = "/root/zjm/SocialNav2/pretrained_model/falcon_noaux_25.pth"
        try:
            ...
            def _extract_state_dict(obj):
                ...
                if isinstance(obj, dict):
                    ...
                    # Multi-agent wrappers (e.g., {0: {state_dict}, ...})
                    for k in (0, '0'):
                        if k in obj and isinstance(obj[k], dict):
                            sd = obj[k].get('state_dict') or obj[k].get('model_state_dict')
                            if isinstance(sd, dict):
                                return sd
                # Fallback: if already a flat dict of tensors (heuristic)
                if isinstance(obj, dict):
                    tensor_like = sum(1 for v in obj.values() if torch.is_tensor(v))
                    if tensor_like >= 20:
                        return obj
                return None

            habitat_state_dict = _extract_state_dict(habitat_checkpoint)
            # Debug prints for visibility
            if isinstance(habitat_checkpoint, dict):
                top_keys = list(habitat_checkpoint.keys())
                print(f"   Top-level keys: {top_keys[:10]} (n={len(top_keys)})")
            if habitat_state_dict is None:
                print("   ✗ Could not locate a valid state_dict inside checkpoint")
                habitat_state_dict = {}
                return
            sample_keys = list(habitat_state_dict.keys())
            print(f"   ✓ Extracted state_dict with {len(habitat_state_dict)} keys")
            if sample_keys:
                print(f"   sample: {sample_keys[:5]}")
        ...
        for key in current_state_dict.keys():
            ...
            if key == 'action_head.weight':
                # Try common prefixes (with/without actor_critic., with/without module.)
                for habitat_key in (
                    'actor_critic.action_distribution.linear.weight',
                    'action_distribution.linear.weight',
                    'module.actor_critic.action_distribution.linear.weight',
                    'module.action_distribution.linear.weight',
                ):
                    if habitat_key in habitat_state_dict:
                        ...
                        if habitat_weight.shape == current_shape:
                            ...
                            break
                if not loaded:
                    print(f"   Action head weight not found in ckpt")
            elif key == 'action_head.bias':
                for habitat_key in (
                    'actor_critic.action_distribution.linear.bias',
                    'action_distribution.linear.bias',
                    'module.actor_critic.action_distribution.linear.bias',
                    'module.action_distribution.linear.bias',
                ):
                    if habitat_key in habitat_state_dict:
                        ...
                        if habitat_bias.shape == current_shape:
                            ...
                            break
                if not loaded:
                    print(f"   Action head bias not found in ckpt")
            ...
            # Handle prev_action_embedding (no dimension adjustment needed - shapes match!)
                ...
                # Support both actor_critic.net.* and net.* (and with module.)
                for habitat_key in (
                    'actor_critic.net.prev_action_embedding.weight',
                    'net.prev_action_embedding.weight',
                    'module.actor_critic.net.prev_action_embedding.weight',
                    'module.net.prev_action_embedding.weight',
                ):
                    if habitat_key in habitat_state_dict:
                        habitat_weight = habitat_state_dict[habitat_key]
                        if habitat_weight.shape == current_shape:
                ...
                if not loaded:
                    print("   Prev action embedding not found in ckpt")
            
            # Handle custom_lstm weights mapping
            elif key.startswith('custom_lstm.'):
                # Map custom_lstm.xxx -> (actor_critic.)?net.state_encoder.rnn.xxx (with optional module.)
                lstm_key = key.replace('custom_lstm.', '')
                for habitat_key in (
                    f'actor_critic.net.state_encoder.rnn.{lstm_key}',
                    f'net.state_encoder.rnn.{lstm_key}',
                    f'module.actor_critic.net.state_encoder.rnn.{lstm_key}',
                    f'module.net.state_encoder.rnn.{lstm_key}',
                ):
                    if habitat_key in habitat_state_dict:
                        habitat_weight = habitat_state_dict[habitat_key]
                        if habitat_weight.shape == current_shape:
                            ...
                            break
                if not loaded:
                    print(f"   Custom LSTM param not found for {key}")
            ...
            # Handle other weights with standard mapping
                ...
                if key in habitat_state_dict:
                    ...
                    # Our model: core_network.xxx -> Habitat: (module.)?(actor_critic.)?net.xxx
                    for base in (
                        'actor_critic.net.',
                        'net.',
                        'module.actor_critic.net.',
                        'module.net.',
                    ):
                        habitat_key = key.replace('core_network.', base)
                        if habitat_key in habitat_state_dict:
                            habitat_weight = habitat_state_dict[habitat_key]
                            if hasattr(habitat_weight, 'shape') and habitat_weight.shape == current_shape:
                                unified_state_dict[key] = habitat_weight
                                loaded_from_habitat += 1
                                loaded = True
                                break
                            else:
                                if hasattr(habitat_weight, 'shape'):
...
class SocialNav2Dataset(Dataset):
    ...
    def __getitem__(self, index):
        if self.mode == 'train':
            # ------------------------------------------------------------------
            # TRAINING: Random episode/timestep sampling for a 3-frame window
            # ------------------------------------------------------------------
...
def validate(model, val_dataloader, device, is_ddp=False):
    ...
    with torch.no_grad():
        for batch_idx, batch_data in enumerate(val_dataloader):
            ...
            for t in range(seq_len):
                ...
                # Calculate loss/acc only at last step (burn-in for first two steps)
                if t == seq_len - 1:
    ...
    # Calculate final metrics (only last step counted)
...
def train():
    ...
    # Optionally freeze backbone before counting/optim setup
    if FREEZE_BACKBONE:
        model_to_edit = model.module if is_ddp else model
        ve = model_to_edit.core_network.visual_encoder
        # Freeze trunk; fall back to whole encoder if no explicit .backbone
        target = ve.backbone if hasattr(ve, 'backbone') else ve
        target.requires_grad_(False)
        if is_main_process:
            print("[FREEZE] visual trunk frozen (requires_grad=False)")
    ...
    if FREEZE_BACKBONE:
        # Build optimizer for top modules only
        m = model.module if is_ddp else model
        param_groups = []
        if hasattr(m.core_network, 'prev_action_embedding'):
            param_groups.append({'params': m.core_network.prev_action_embedding.parameters(), 'lr': LR_TOP, 'weight_decay': WD_TOP})
        param_groups.append({'params': m.custom_lstm.parameters(), 'lr': LR_TOP, 'weight_decay': WD_TOP})
        param_groups.append({'params': m.action_head.parameters(), 'lr': LR_TOP, 'weight_decay': WD_TOP})
        optimizer = optim.AdamW(param_groups, eps=1e-8)
        if is_main_process:
            trainable_param_count = sum(p.numel() for g in param_groups for p in g['params'])
            print(f"[OPT] Using frozen-backbone optimizer. Trainable params: {trainable_param_count:,}")
    elif USE_GROUPED_BACKBONE_LR:
        # Fully unfrozen; small LR for trunk, larger LR for heads/top modules
        m = model.module if is_ddp else model
        ve = m.core_network.visual_encoder
        trunk = ve.backbone if hasattr(ve, 'backbone') else ve

        param_groups = [
            { 'params': trunk.parameters(), 'lr': LR_BACKBONE, 'weight_decay': WD_BACKBONE }
        ]
        # Top/heads
        if hasattr(m.core_network, 'prev_action_embedding'):
            param_groups.append({ 'params': m.core_network.prev_action_embedding.parameters(), 'lr': LR_TOP, 'weight_decay': WD_TOP })
        if hasattr(ve, 'compression'):
            param_groups.append({ 'params': ve.compression.parameters(), 'lr': LR_TOP, 'weight_decay': WD_TOP })
        if hasattr(m.core_network, 'visual_fc'):
            param_groups.append({ 'params': m.core_network.visual_fc.parameters(), 'lr': LR_TOP, 'weight_decay': WD_TOP })
        param_groups.append({ 'params': m.custom_lstm.parameters(), 'lr': LR_TOP, 'weight_decay': WD_TOP })
        param_groups.append({ 'params': m.action_head.parameters(), 'lr': LR_TOP, 'weight_decay': WD_TOP })
        ...
        if is_main_process:
            ...
            print(f"[OPT] Grouped LR. Params: {total_trainable:,} | trunk LR={LR_BACKBONE}, top LR={LR_TOP}")
    else:
    ...
    if SCHEDULER_TYPE == "plateau":
        ...
        schedulers.append(lr_scheduler.CosineAnnealingLR(
            optimizer,
            T_max=max(1, NUM_EPOCHS - max(0, WARMUP_EPOCHS)),
            eta_min=COSINE_MIN_LR,
        ))
        if len(milestones) > 0:
            scheduler = lr_scheduler.SequentialLR(optimizer, schedulers=schedulers, milestones=milestones)
        else:
            scheduler = schedulers[-1]
        if is_main_process:
            print(f"[LR] Warmup+Cosine | warmup={WARMUP_EPOCHS} ep, start_factor={WARMUP_START_FACTOR}, eta_min={COSINE_MIN_LR}")
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for batch_idx, batch_data in enumerate(train_dataloader):
            ...
            for t in range(seq_len):
                ...
                # Autoregressive: use model prediction as next-step input
                predicted_actions = torch.argmax(action_logits, dim=1)

                # Only last step contributes to loss/accuracy (burn-in for first two steps)
                if t == seq_len - 1:
            ...
            batch_accuracy = correct_predictions / max(1, total_predictions)
        ...
        if scheduler is not None:
            if SCHEDULER_TYPE == "plateau":
                metric_for_sched = val_loss if val_loss is not None else avg_epoch_loss
                if is_ddp:
                    # Broadcast metric from rank 0 to all ranks for consistent stepping
                    if torch.distributed.is_initialized():
                        if is_main_process:
                            metric_tensor = torch.tensor(metric_for_sched, device=DEVICE)
                        else:
                            metric_tensor = torch.zeros(1, device=DEVICE)
                        torch.distributed.broadcast(metric_tensor, src=0)
                        metric_for_sched = float(metric_tensor.item())
        ...
        if is_main_process:
            ...
            if SAVE_BEST and can_save_best and is_better:
                ...
                if writer is not None:
                    ...
                    # Record the actual best metric value, not training metrics
                    if SAVE_BEST_BY == "train_loss":
                        writer.add_scalar("best/loss", best_metric, epoch + 1)
                        writer.add_scalar("best/accuracy", avg_epoch_accuracy, epoch + 1)
                    elif SAVE_BEST_BY == "train_accuracy":
                        writer.add_scalar("best/loss", avg_epoch_loss, epoch + 1)
                        writer.add_scalar("best/accuracy", best_metric, epoch + 1)
                    elif SAVE_BEST_BY == "val_loss":
                        writer.add_scalar("best/loss", best_metric, epoch + 1)
                        writer.add_scalar("best/accuracy", val_accuracy, epoch + 1)
                    elif SAVE_BEST_BY == "val_accuracy":
                        writer.add_scalar("best/loss", val_loss, epoch + 1)
                        writer.add_scalar("best/accuracy", best_metric, epoch + 1)
