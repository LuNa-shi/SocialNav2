...
# Dataset paths
DATASET_ROOT = "/data/il_dataset/expert_data_test"
...
SAMPLES_PER_EPOCH = 10000   # Total sample windows per epoch (controls __len__)
...
VAL_EPISODES_COUNT = 2        # Number of episodes for validation (P0: use 2 episodes)
...
# Output paths
MODEL_SAVE_PATH = "checkpoints0821/il_model_0821.pth"
CHECKPOINT_DIR = "checkpoints0821"
TENSORBOARD_DIR = "runs/il_trainer_test"
...
# ------------------------------------------------------------------------------
# VALIDATION FUNCTION (AUTOREGRESSIVE): prev_action comes from model prediction
# ------------------------------------------------------------------------------
def validate_autoreg(model, val_dataloader, device, is_ddp=False):
    """
    Run autoregressive validation: use model predictions as prev_actions (AR).
    ...
    with torch.no_grad():
        for batch_idx, batch_data in enumerate(val_dataloader):
            observations_batch = batch_data["observations"]
            ...
            for t in range(seq_len):
                current_observations = {key: val[:, t].to(device) for key, val in observations_batch.items()}
                current_actions = actions_batch[:, t].to(device)
                current_masks = masks_batch[:, t].to(device)
                if t == 0:
                    current_masks = torch.zeros_like(current_masks)
                ...
                # AR: feed model predictions to next step
                prev_actions_batch = predicted_actions

            if batch_idx % 50 == 0:
                print(f"[AR] Validation batch {batch_idx}/{len(val_dataloader)}")
...
def train():
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if (epoch + 1) % VAL_INTERVAL == 0 and is_main_process:
            ...
            # Teacher-Forcing validation (prev_action = GT)
            val_tf_loss, val_tf_acc = validate(model, val_dataloader, DEVICE, is_ddp)

            # Autoregressive validation (prev_action = model prediction)
            val_ar_loss, val_ar_acc = validate_autoreg(model, val_dataloader, DEVICE, is_ddp)

            # Choose AR as main selection metric, but log both
            val_loss, val_accuracy = val_ar_loss, val_ar_acc

            print("Validation Results:")
            print(f"  TF  - Loss: {val_tf_loss:.6f} | Acc: {val_tf_acc:.4f}")
            print(f"  AR  - Loss: {val_ar_loss:.6f} | Acc: {val_ar_acc:.4f} (used for selection)")
            print(f"{'='*60}")

            # TensorBoard logging
            if writer is not None:
                writer.add_scalar("validation_TF/loss", val_tf_loss, epoch + 1)
                writer.add_scalar("validation_TF/accuracy", val_tf_acc, epoch + 1)
                writer.add_scalar("validation_AR/loss", val_ar_loss, epoch + 1)
                writer.add_scalar("validation_AR/accuracy", val_ar_acc, epoch + 1)
